...
class KubernetesApi extends ApiImplementation {
    ...
    createService(k8s_app_desc, k8s_ports, discovery_ports, cb) {
        ...
        return request(req, (err, res) => {
            if(err || res.statusCode !== 201) {
                return cb(`Could not create service: ${service_body.metadata.name} for application: ${app_name}`);
            }
    ...
    getApplication(application, cb) {
        return this.getApplications((err, apps) => {
            ...
            if(!app) {
                return cb(`The requested application does not exist: ${application}`);
            }
    ...
    getContainers(cb) {
        return request(`${this.k8s_api_url}/api/v1/pods`,
            (err, res, body) => {
                if(err || res.statusCode !== 200) {
                    return cb('Failed to retrive k8s pods inside getContainers call');
                }
    ...
    createApplication(app_desc, cb) {
        ...
        return request({
            ...
            if(err || res.statusCode !== 201) {
                return cb(`Could not create replication controller for ${app_desc.id}`);
            }
        ...
        return this.updateApplication(name, { tags: { constraints: {} }, count: 0 }, (err) => {
            ...
            function safeDeleteRC(delete_cb) {
                return request({
                    ...
                    if(pod_count > 0) {
                        return delete_cb(`There are still pods: ${pod_count}...waiting 4 seconds for pods to delete`);
                    }
    ...
    getHost(host, cb) {
        return this.getHosts((err, hosts) => {
            ...
            if(!hosts[host]) {
                return cb(`The requested host does not exist: ${host}`);
            }
    ...
    updateHost(host_id, tags, cb) {
        if(!tags) {
            return cb('You must provide tags to update on the host');
        }
